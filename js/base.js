...
Date.prototype.pattern = function(fmt)
{
    var o = {
        ...
        'd+' : this.getDate(), //日
        'h+' : this.getHours()%12 == 0 ? 12 : this.getHours()%12, //小时（12时制）
        'H+' : this.getHours(), //小时（24时制）
        'm+' : this.getMinutes(), //分
        's+' : this.getSeconds(), //秒
    ...
    for(var k in o){
        if(new RegExp('('+ k +')').test(fmt)){
            if (k == 'S+') fmt = fmt.replace(RegExp.$1, (o[k] == 0) ? '000' : (''+ o[k]));
            else fmt = fmt.replace(RegExp.$1, (RegExp.$1.length==1) ? (''+ o[k]) : (('00'+ o[k]).substr((''+ o[k]).length)));
        }
    }
    return fmt;
};
